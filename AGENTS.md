...
4. Center the work in `research-brief.md`, with `sources.csv`, `extractions.csv`, `source-cache/manifest.csv`, and `adversarial-evaluation.md` as the default supporting machinery. Use `appendices/` only for genuinely long legal, data, source, or case readouts.
...
7. For follow-ups, add an update file under `updates/` and mutate `research-brief.md`, `sources.csv`, `extractions.csv`, source-cache records, and appendices as needed. Do not append free-floating essay chapters.
