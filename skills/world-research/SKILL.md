...
Use this skill as a writer research desk for current events, past events, policy, economics, science-in-policy disputes, and history. The default aim is to understand the issue deeply enough to help the user write, edit, or decide with authority. Do not optimize for answer-shaped prose; optimize for source comprehension, claim discipline, and article usefulness inside the stated boundary.

## Non-negotiables

- A polished but shallow project or dossier is a failed draft. Do not deliver it as a completed answer.
- The primary task is research, not proving or disproving the user's prior. Understand the source record first; then explain what the evidence earns, weakens, complicates, or leaves unknown for the user's article or op-ed.
- The packet must educate the user, not only score claims. A deliverable packet must explain the issue's history, institutions, source record, technical machinery, and causal reasoning in plain English well enough that a curious reader can understand why each claim is supported, weakened, contradicted, or unresolved. Tables and CSVs are audit rails, not substitutes for teaching.
- Coverage is not the enemy of depth; shallow coverage is. Read as many sources as are necessary, as deeply as each source's importance requires, to reach full understanding inside the agreed boundary. If that cannot be done in one pass, narrow the boundary or mark the work incomplete.
- Every pass must start by asking: What must be read to understand this? Which sources are load-bearing? Have those sources been read deeply enough to explain them better than a media summary? What source class is missing? What would change the assessment? Am I stopping because the issue is understood, or because enough artifacts exist?
- Treat adversarial review as a research loop, not a deliverable. If a hostile professor, expert editor, or PhD examiner would not accept the packet inside the stated boundary, return to source acquisition, source readout, extraction, and revision. Do not resolve blocking critiques with caveats.
...
- No synthesis before evidence-thread completion. First split the research into central evidence threads, record them in `evidence-threads.csv`, and define the completion standard and required primary sources for each. Do not write a settled conclusion, mark an angle promising/ready, or call the project deliverable until every central thread is complete or the project is explicitly labeled incomplete inside the chosen boundary.
- No synthesis before extraction. Do not write a settled conclusion until load-bearing claims have rows in `extractions.csv` or have been explicitly marked unauditable/incomplete. Do not use search-result snippets, previews, abstracts, landing pages, or secondary summaries as central evidence when a primary document, filing, dataset, transcript, docket, report, video, audio, or source page is reasonably available.
...
- The evaluator's `VERDICT: pass` is invalid if central evidence threads are incomplete, blocking source-acquisition targets are pending, source readouts are thin, exact quotes rely on AI/aggregator/snippet/secondary material while primary audio/video/transcript is reasonably available, or the packet claims article readiness before the source record is understood.
...
- When the user asks a follow-up, second pass, continuation, gap fill, deeper audit, or revision pass on an existing project, the default is a **targeted update pass**. Keep the same project folder, leave prior core artifacts intact, and create distinct update-scoped artifacts instead of making the reader parse changed originals. Initialize substantial passes with `scripts/init_update_pass.py <project-dir> --slug <follow-up-slug> --question "<question>"`. The pass must include `<slug>-writer-research-packet.md`, `<slug>-evidence-threads.csv`, `<slug>-source-acquisition.csv`, `<slug>-sources.csv`, `<slug>-extractions.csv`, `<slug>-source-cache-manifest.csv`, `<slug>-adversarial-evaluation.md`, and `updates/<YYYY-MM-DD>-<slug>.md`. The update must clearly state whether and how it changes prior conclusions, confidence levels, source assessments, or claim status. Do not overwrite, silently mutate, or "clean up" earlier packet/source/extraction/cache/evaluator artifacts unless the user explicitly asks for a consolidated revision.
...
- Turn source readouts into reader education. When a central source contains history, institutional background, foreign-language official claims, technical procedures, budget mechanics, or causal explanations, summarize and explain the source's substance in the packet before using it. Do not merely say a source "supports" a claim; explain what the source says, what the words mean, what mechanism it implies, what is missing, and how that changes the reader's understanding.
- For foreign-language sources, translate and explain the relevant source content in plain English. Include short translated excerpts when useful, identify the original language, and explain terms of art, agency names, legal/administrative actions, and context the user may not know. A citation to a foreign-language page without an English explanation is incomplete when the source is load-bearing.
...
- Source readouts are mandatory for load-bearing sources. A readout must explain what the source is, what parts were read, the source's research question or institutional purpose, data/method/evidence, what it actually says, what it does not prove, strengths, weaknesses, funding/ownership/incentives, what media or advocates get right or overstate, and how an article can use it. CSV rows are audit rails; they do not substitute for source comprehension.
- `sources.csv` must label each source's access depth and audit state. `source_access` describes what was accessed: `full_document`, `partial_document`, `dataset_inspected`, `filing_inspected`, `docket_inspected`, `transcript_inspected`, `secondary_summary`, or `snippet_only`. `audit_state` describes what has actually happened: `found`, `opened`, `audited`, or `publication_ready`. Central claims may be supported only by sources marked `audited` or `publication_ready`. `found` and `opened` sources belong in the reporting plan, not the evidence backbone. `snippet_only` may orient the search but may not support central claims. `secondary_summary` may not support central claims when a primary source is reasonably available.
- Maintain `source-acquisition.csv` for source targets. Blocking source targets are records, studies, transcripts, datasets, videos/audio, dockets, or expert/primary materials whose absence prevents real understanding inside the boundary. Pending blocking targets prevent usable/strong research readiness and promising/ready article status.
...
- Explain the reasoning behind verdicts. For each central claim, include a plain-English "why this is judged this way" explanation that connects chronology, mechanism, source quality, alternative explanations, and what evidence would change the assessment. Do not leave the user to infer the reasoning from `sources.csv` or `extractions.csv`.
...
2. Set the depth contract:
   ...
   - **Full Writer Research Desk / Evidence Audit Project:** default for broad, consequential, controversial, scientific, economic, historical, policy, or current-events questions. Create a folder centered on `writer-research-packet.md`, supported by `source-acquisition.csv`, `evidence-threads.csv`, `sources.csv`, `extractions.csv`, `source-cache/manifest.csv`, appendices only when needed, updates, and adversarial evaluation.
3. Initialize the project:
   - New project: run `scripts/init_evidence_audit.py research/<topic-slug> --question "<question>"`.
   - Existing project follow-up, second pass, continuation, or gap fill: default to a targeted update pass. Run `scripts/init_update_pass.py <project-dir> --slug <slug> --question "<question>"`. Keep the original packet/source/extraction/cache/evaluator files unchanged unless the user explicitly asks for a consolidated revision.
4. Build the assignment brief and source universe before narrowing:
   - Identify what the user is trying to write or understand.
   - Identify every source class needed for understanding: primary records, studies/data, legal filings, transcripts/audio/video, official datasets, strongest critics, strongest defenders, specialist literature, local records, and context reporting.
   - Rank sources by function: load-bearing, adversarial, contextual, duplicative, or lead-only.
   - Record concrete source targets in `source-acquisition.csv` or `<slug>-source-acquisition.csv` with minimum depth and whether missing evidence blocks synthesis.
5. Build the evidence-thread contract before narrowing:
   - Break the user's request into central evidence threads, not just sections of prose.
   - For each thread, define the question parts it answers, whether it is central, completion standard, required primary sources, required artifact, status, blocking gap, completion evidence, and whether synthesis is allowed.
   - Record this in `evidence-threads.csv` and mirror the central thread contract in `writer-research-packet.md`.
   - Examples: legal-record thread, source-video thread, legislative-history thread, empirical-literature thread, comparison-case thread, current-status thread, viewpoint-intersection thread.
   - A central thread is complete only when its required primary/near-primary sources are audited and its completion evidence cites claim/source IDs that exist in `extractions.csv`. If a central thread is partial, blocked, or incomplete, the project cannot be marked deliverable and the packet cannot present a settled synthesis.
6. Build an issue and claim universe before narrowing:
...
7. Fill the opening sections of `writer-research-packet.md` before source extraction:
   - Split the user's prompt into concrete subquestions inside Question And Boundary and Assignment Brief.
   - Add Source Universe Map, Source Acquisition Plan, Evidence Thread Contract, Orientation, Timeline, System Explainer, Evidence Backbone, Live Viewpoints, Economic Perspectives when relevant, Causal Models, Emerging Tensions, Angle Readiness, Claims To Avoid, So What?, Reporting Plan, Writer's Current Position, Bias And Symmetry Check, Hostile Editor Review, Adversarial Review Loop, Quality Gate Result, and Expert Evaluator Result.
...
9. Build an explanatory map:
   ...
   - Add an "Education Brief" or equivalent narrative section for major reader questions such as "what is the history?", "why did this happen?", "why was it not stopped?", and "what would have to be true for the claim to be right?" This section must teach from the audited sources and cite claim/source anchors.
   - If the user is unlikely to understand a foreign-language source, agency, legal action, or technical term, translate and explain it before relying on it.
10. Build a source plan using this priority order, while preserving source neutrality:
...
11. Fill `sources.csv` and `source-cache/manifest.csv`:
   - Record source id, title, URL/citation, source type, viewpoint, incentives/funding/ownership, claims made, data access, `source_access`, `audit_state`, and whether it is eligible for central evidence.
   - Use `audit_state=found` only for located leads, `opened` for accessed but not context-audited sources, `audited` for sources whose relevant context has been read and extracted, and `publication_ready` for sources with direct verifiable URL/citation, pinpoint location, context checked, and claim-fit tested.
   - Central evidence requires `audit_state=audited` or `publication_ready`. If a central source is only `found` or `opened`, move it to the reporting plan and mark the evidence thread incomplete.
   - Record access depth and source-cache status for load-bearing sources.
12. Write primary/load-bearing source readouts before synthesis:
   - For studies: research question, dataset, sample, unit of analysis, period, variables/measures, model/design, effect sizes, uncertainty, robustness, limitations, funding/incentives, and what media or advocates overstate.
   - For legal records: procedural posture, holding/rule, evidence discussed, application, dicta, what was not decided, strongest attack, and article use.
   - For transcripts/audio/video: provenance, exact words with timestamp/page, surrounding context, whether the quote is primary/official/AI/aggregator, tone limits, fair criticism, unfair overclaim, and article use.
   - For datasets: source, codebook, rows/cases inspected, missingness, denominator, coding choices, sensitivity, and what can and cannot be inferred.
   - For comparison cases: official finding, what was proven, what was alleged, what remains unproven, classification standard, and safe article use.
13. Fill `extractions.csv` before synthesis:
...
12. For historical claims, comparisons, or analogies, build a historical context module before answering:
...
- `writer-research-packet.md`: the coherent working packet and final output. It contains question/boundary, assignment brief, source universe map, source acquisition plan, evidence thread contract, orientation, timeline, system explainer, evidence backbone with source readouts, live viewpoints, economic perspectives when relevant, causal models, emerging tensions, angle readiness, claims to avoid, So What stress test, reporting plan, writer's current position, bias/symmetry check, hostile editor review, adversarial review loop, unknowns, quality gate result, and expert evaluator result.
- `writer-research-packet.md` must also include a reader-facing explanatory narrative when the user is trying to learn the subject. This can be titled `Education Brief`, `How It Works`, `History And Mechanism`, or another clear title, but it must answer the user's natural "why/how/history" questions in prose with source anchors. A packet that only provides ledgers, verdict tables, or source-card summaries is not deliverable for research meant to educate the user.
- `evidence-threads.csv`: the completion contract for the project. Each central evidence thread must name its completion standard, required primary sources, required artifact, status, blocking gap, completion evidence, and whether synthesis is allowed.
- `source-acquisition.csv`: source-target ledger for primary records, studies, datasets, transcripts, audio/video, dockets, and adversarial/source-class gaps. Blocking targets that remain pending prevent positive readiness.
- `sources.csv`: source inventory from all viewpoints, with incentives, access depth, audit state, primary-source availability/use, centrality, evidence limits, `economic_lens`, `representative_status`, and `issue_specificity`.
...
- `updates/`: follow-up questions as evidence-model updates, not mini-essays. Substantial follow-ups should also have their own slugged packet/source/extraction/cache/evaluator files in the same project folder so readers do not need to diff or re-parse the original artifacts.
...
For follow-up questions, second passes, continuations, or gap fills on the same topic, keep the original project folder. Use `scripts/init_update_pass.py <project-dir> --slug <slug> --question "<question>"`, add a dated update file, and create distinct update-scoped artifacts with the same slug. Treat the original packet/source/extraction/cache/appendix/evaluator artifacts as immutable historical records of that pass. Do not overwrite earlier analysis, silently mutate original artifacts, or fold the update into the main packet unless the user explicitly asks for a consolidated revision. If the user later wants a merged final version, create a new consolidated file that cites the original and follow-up artifacts.
...
Then run `scripts/research_quality_gate.py <project-dir-or-dossier-path>` for full projects or `scripts/research_quality_gate.py <project-dir> --pass <slug>` for update passes. Treat any failure as a block on delivery unless the final response clearly says the project is incomplete or failed.
