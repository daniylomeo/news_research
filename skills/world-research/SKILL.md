...
- When the user asks a follow-up question about an existing project, keep the same project folder but create distinct follow-up artifacts instead of making the reader parse changed originals. Add `updates/<YYYY-MM-DD>-<follow-up-slug>.md` plus update-scoped files such as `<follow-up-slug>-writer-research-packet.md`, `<follow-up-slug>-sources.csv`, `<follow-up-slug>-extractions.csv`, `<follow-up-slug>-source-cache-manifest.csv`, and `<follow-up-slug>-adversarial-evaluation.md` when the follow-up is substantial. The update must clearly state whether and how it changes prior conclusions, confidence levels, source assessments, or claim status. Do not overwrite or silently mutate earlier analysis unless the user explicitly asks for a consolidated revision.
...
3. Initialize the project:
   ...
   - Existing project follow-up: add an update file under `updates/` and create update-scoped packet/source/extraction/cache/evaluator artifacts with a shared slug. Revise the prior verdict only in the update artifact unless the user asks for a consolidated revision.
...
For follow-up questions on the same topic, keep the original project folder. Add a dated update file and create distinct update-scoped artifacts with the same slug. Do not overwrite earlier analysis or silently mutate the original packet/source/extraction/cache/appendix artifacts. If the user later wants a merged final version, create a new consolidated file that cites the original and follow-up artifacts.
